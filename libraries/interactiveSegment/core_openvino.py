...
class Resize(object):
    def __init__(self, 
                 size, 
                 mode=None,  
                 elems_point=['pos_points_mask','neg_points_mask','first_point_mask'], 
                 elems_do=None, 
                 elems_undo=[]):
    ...
    def __call__(self, sample):
        for element in sample.keys():
            if self.elems_do != None and element not in self.elems_do:
                continue
            if element in self.elems_undo:
                continue
            if element in self.elems_point:
                sample[element] = img_resize_point(sample[element], self.size)
                continue
            if self.mode is None:
                mode = cv2.INTER_LINEAR if len(sample[element].shape) == 3 else cv2.INTER_NEAREST
            sample[element] = cv2.resize(sample[element], self.size, interpolation=mode)
        return sample
...
class CatPointMask(object):
    ...
    def __call__(self, sample):
        ...
        if self.mode == 'DISTANCE_POINT_MASK_SRC':
            max_dist = 255
            if if_gt_empty:
                pos_points_mask_dist = np.ones(gt.shape, dtype = np.float64) * max_dist
            else:
            ...
            if neg_points_mask.any() == False:
                neg_points_mask_dist = np.ones(gt.shape, dtype=np.float64) * max_dist
            else:
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        for elem in sample.keys():
            if self.elems_do != None and elem not in self.elems_do:
                continue
...
def predict(model, img, seq_points, output_keys, if_sis=False, if_cuda=True):
    ...
    sample = dict()
    sample['img'] = img.copy()
    sample['gt'] = np.ones((h, w), dtype=np.uint8) * 255
