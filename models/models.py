...
def create_modules(module_defs, img_size, cfg):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            filters = mdef['filters']
            k = mdef['size']  # kernel size
            modules = CBAMBlock(channel= filters,
                                kernel_size= k,
                                reduction=16)
            ...
            rates = mdef['layers']
            activation = True
            modules = ASPP(in_channel=output_filters[-1],
                            out_channel= filters,
                            rates=rates, relu= activation)
                            
        elif mdef['type'] == 'aspp2':
            filters = mdef['filters']
            rates = mdef['layers']
            if len(rates) >3:
                rates = rates[:2]
            activation = True
            modules = ASPP_concat(in_channel=output_filters[-1],
                            out_channel= filters,
                            rates=rates, relu= activation)
                            
        elif mdef['type'] == 'rfa':
            filters = mdef['filters']
            layers = mdef['layers'] if len(mdef['layers']) >0 else [1,2,3]
            layers = [l/1.0 for l in layers]
            modules = RFA(in_channels = output_filters[-1],
                            out_channels = filters,
                            pool_ratios = layers)
