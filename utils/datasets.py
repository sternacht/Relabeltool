...
def augment_hsv_2(img, hgain=0.5, sgain=0.5, vgain=0.5):
...
def augment_hsv(img, hgain=0.1, sgain=1.5, vgain=1.5):
    hue = rand(-hgain, hgain)
    sat = rand(1, sgain) if rand()<.5 else 1/rand(1, sgain)
    val = rand(1, vgain) if rand()<.5 else 1/rand(1, vgain)
    x = cv2.cvtColor(np.array(img ,np.float32)/255, cv2.COLOR_BGR2HSV)
    x[..., 0] += hue*360
    x[..., 0][x[..., 0]>1] -= 1
    x[..., 0][x[..., 0]<0] += 1
    x[..., 1] *= sat
    ...
    x[:, :, 1:][x[:, :, 1:]>1] = 1
    x[x<0] = 0
    cv2.cvtColor(x, cv2.COLOR_HSV2BGR, dst=img) # numpy array, 0 to 1
