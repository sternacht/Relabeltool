...
class NoduleTracker():
    ...
    def update(self, 
        ...
        if len(self.tracked_nodule_bbox2ds) == 0:
            ...
            position = np.argwhere(iou_matrix > self.__iou_threshold)
            ...
            for (row, col) in position:
                if row in usedRows or col in usedCols:
                    continue
                nodule_id = tracked_nodule_ids[row]
                self.tracked_nodule_bbox2ds[nodule_id] = nodule_bbox2d_infos[col]
                self.tracked_nodule_slice_ids[nodule_id] = slice_i
                self.__disappeared[nodule_id] = 0
                usedRows.add(row)
                usedCols.add(col)
            unusedRows = set(range(0, iou_matrix.shape[0])).difference(usedRows)
            unusedCols = set(range(0, iou_matrix.shape[1])).difference(usedCols)
            if iou_matrix.shape[0] >= iou_matrix.shape[1]:
                ...
                for col in unusedCols:
                    self.register(nodule_bbox2d_infos[col], slice_i)
...
def collect3d(patient_merged_dict: Dict[str, Dict[int, List[Dict[str, Any]]]]):
    ...
    for patient, slices_merged_dict in patient_merged_dict.items():
        ...
        for slice_i, bboxes in slices_merged_dict.items():
            ...
            if slice_i + 2 not in slices_merged_dict.keys():
                tracked_nodule_bbox2ds, slice_ids = nodule_tracker.update([], slice_i + 2)
