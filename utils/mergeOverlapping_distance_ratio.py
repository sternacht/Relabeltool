...
class NoduleTracker():
    ...
    def update(self, rects: List[Any], num_slice):
        ...
        if len(self.objects) == 0:
            ...
            max_value_along_row = np.max(distance_matrix, axis=1)
            max_value_along_col = np.max(distance_matrix, axis=0)
            argmax_value_along_col = np.argmax(distance_matrix, axis=0)
            
            # Deregiseter old object
            for i in range(len(max_value_along_row)):
                if max_value_along_row[i] >= self.__dcThres:
                    objectID = objectIDs[i]
                    self.__disappeared[objectID] += 1
                    if self.__disappeared[objectID] >= self.__maxdisappeared:
                        self.deregister(objectID)
            # Register new object or update old object
            for old_objectID, new_i in zip(argmax_value_along_col, range(len(max_value_along_col))):
                if max_value_along_col[new_i] < self.__dcThres:
                    self.objects[old_objectID] = rects[new_i]
                    self.slices[old_objectID] = num_slice
                    self.__disappeared[old_objectID] = 0
                else:
                    self.register(rects[new_i], num_slice)
        ...
        return self.objects, self.slices
def collect3d(mergedDict: Dict[str, Dict[int, List[Dict[str, Any]]]]):
    """
    Args:
